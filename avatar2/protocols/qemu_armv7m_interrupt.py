...
class QEmuARMV7MInterruptProtocol(Thread):
    ...
    (b) is necessary in cases where two targets need to be synched on the
        ...
        interrupt number
    :param target:        Reference to the Target utilizing this protocol
    :param rx_queue_name: Name of the queue for receiving
    :param tx_queue_name: Name of the queue for sending
    """

    def __init__(self, target, rx_queue_name, tx_queue_name):
        super(self.__class__, self).__init__()
        self._rx_queue_name = rx_queue_name
        self._tx_queue_name = tx_queue_name
        self._rx_queue = None
        self._tx_queue = None
        self._avatar_queue = target.avatar.fast_queue
        self.target = target
        self._close = Event()
        self._closed = Event()
        self._close.clear()
        self._closed.clear()
        self.log = logging.getLogger(f'{target.log.name}.protocols.{self.__class__.__name__}')
    ...
    def run(self):
        while not self._close.is_set():
            ...
            if RINOperation(req_struct.operation) == RINOperation.ENTER:
                msg = RemoteInterruptEnterMessage(self.target, req_struct.id,
                                                  req_struct.num_irq)
                self.log.warning(
                ...
                msg = RemoteInterruptExitMessage(self.target, req_struct.id,
                                                 req_struct.type,
                                                 req_struct.num_irq)
                self.log.warning(
    ...
    def enable_interrupts(self):
        if isinstance(self.target, QemuTarget):
            ...
            rmem_rx_qname = self.target.protocols.remote_memory.rx_queue_name
            rmem_tx_qname = self.target.protocols.remote_memory.tx_queue_name
        ...
        self.daemon = True
        self.start()
        self.log.info("Enabled Interrupt Forwarding for %s" % self.target)
        return True
